...
export BACKEND_ECR_URI="xxxxx.dkr.ecr.us-east-1.amazonaws.com/event-registry-backend-staging"
export FRONTEND_ECR_URI="xxxxx.dkr.ecr.us-east-1.amazonaws.com/event-registry-frontend-staging"

# Set these after Step 6 (S3)
export BUCKET_NAME="event-registry-staging-uploads-xxxxx"
...
```bash
aws ecs update-service --cluster event-registry-staging --service backend-service --force-new-deployment
aws ecs update-service --cluster event-registry-staging --service frontend-service --force-new-deployment
```

### Update SSM Parameter
```bash
aws ssm put-parameter --name "/event-registry-staging/PARAMETER_NAME" --type "String" --value "new-value" --overwrite
```
