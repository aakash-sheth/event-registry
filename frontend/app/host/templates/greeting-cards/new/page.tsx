...
import api, { uploadImage as uploadEventImage } from '@/lib/api'
...
// ---------------------------------------------------------------------------
// Types (same shape as card/page.tsx TextBox / GreetingCardSample.text_overlays)
// ---------------------------------------------------------------------------
  ...
  const handleCanvasPointerMove = useCallback((e: React.PointerEvent<HTMLDivElement>) => {
    ...
    setTextBoxes((prev) =>
      prev.map((b) => {
        ...
        return { ...b, x: clamp(startBoxX + dx, 0, 100 - b.width), y: clamp(startBoxY + dy, 0, 95) }
      })
    )
  }, [])
        ...
        <div className="lg:w-80 flex-shrink-0 bg-white border-r border-gray-200 p-6 flex flex-col gap-5">
          ...
          <div className="border-t pt-4">
            ...
            <button
              type="button"
              onClick={addTextBox}
              className="w-full py-2 rounded-lg border border-blue-400 text-sm text-blue-600 hover:bg-blue-50 font-medium"
            >
              + Add placeholder text
            </button>
            {textBoxes.length > 0 && (
              <p className="text-xs text-gray-400 mt-1">{textBoxes.length} overlay{textBoxes.length !== 1 ? 's' : ''} — drag to reposition on canvas</p>
            )}
          </div>
        </div>
      ...
      {selectedBox && (
        <div className="sticky bottom-0 z-20 bg-white border-t border-gray-200 px-4 py-2 flex items-center gap-2 flex-wrap overflow-x-auto">
          ...
          {(['bold', 'italic', 'underline', 'strikethrough'] as const).map((prop) => (
            <button key={prop} onClick={() => toggleProp(selectedBox.id, prop)} className={`px-2 py-1 rounded text-sm transition-colors ${selectedBox[prop] ? 'bg-gray-200' : 'hover:bg-gray-100'}`} title={prop}>
              {prop === 'bold' ? <b>B</b> : prop === 'italic' ? <i>I</i> : prop === 'underline' ? <u>U</u> : <s>S</s>}
            </button>
          ))}
          <div className="w-px h-5 bg-gray-200 flex-none" />
          {(['left', 'center', 'right'] as const).map((align) => (
            <button key={align} onClick={() => updateBox(selectedBox.id, 'textAlign', align)} title={`Align ${align}`} className={`px-2 py-1 rounded text-sm transition-colors ${selectedBox.textAlign === align ? 'bg-gray-200' : 'hover:bg-gray-100'}`}>
              {align === 'left' ? '←' : align === 'center' ? '↔' : '→'}
            </button>
          ))}
          <div className="w-px h-5 bg-gray-200 flex-none" />
          <div className="flex items-center gap-1">
            <input type="color" value={selectedBox.color} onChange={(e) => updateBox(selectedBox.id, 'color', e.target.value)} className="w-8 h-8 rounded border border-gray-300 cursor-pointer p-0.5" title="Text color" />
          ...
          <div className="w-px h-5 bg-gray-200 flex-none" />
          <button onClick={() => deleteBox(selectedBox.id)} className="text-red-500 hover:bg-red-50 px-2 py-1 rounded text-sm transition-colors" title="Delete text box">Delete</button>
        </div>
      )}
    </div>
  )
