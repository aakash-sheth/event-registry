...
class EventViewSet(viewsets.ModelViewSet):
        ...
        def guests(self, request, id=None):
            ...
            if request.method == 'GET':
                ...
                # Include both active and removed RSVPs to properly separate them
        ...
        def get_overall_impact(self, request):
            ...
            for event in expired_events:
                ...
                if impact:
                    ...
                    expiry = getattr(event, 'expiry_date', None)
        ...
        def retrieve(self, request, *args, **kwargs):
            ...
            try:
                ...
                try:
                    ...
                    if not invite_page.is_published:
                        query_time = time.time() - query_start
                        logger.error(
                            f"INVITE_404: Unpublished invite page accessed",
                            extra={
            ...
            if event.event_structure == 'SIMPLE' and not guest_token:
                ...
                if guest_token:
                    ...
                    # First check if event has any sub-events at all (fast count query)
                    ...
                    if not has_any_subevents:
                        ...
                        # Event has sub-events - query for public-visible ones
                        allowed_sub_events = SubEvent.objects.filter(
                            event=event,
                ...
                # SECURITY FIX: Do NOT auto-upgrade event structure in public GET endpoint
                # This violates REST idempotency and can cause race conditions
                # Event structure should be upgraded through authenticated admin endpoints
                # Note: If event structure is SIMPLE but sub-events exist, the serializer
                # will handle this gracefully without modifying the database
            ...
            if elapsed_time > 1.0:
                logger.warning(f"[PublicInviteViewSet] SLOW REQUEST: {elapsed_time:.2f}s (sub-events: {sub_events_total_time:.2f}s) for slug: {slug}")
